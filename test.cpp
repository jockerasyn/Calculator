...
int main()
{
...

    {
        ...
        std::atomic_int thread_finished = 0;
        std::thread t1(sum_of_arr_atomic(), std::ref(array), 0, int(round(array.size() / 2) + 1), std::ref(sum), std::ref(thread_finished));
        std::thread t2(sum_of_arr_atomic(), std::ref(array), int(round(array.size() / 2) + 1), int(array.size()), std::ref(sum), std::ref(thread_finished));
        while (true)
        {
            if (thread_finished == 2)
            {
                std::cout << "sum of array with atomic = " << sum << std::endl;
                break;
            }
        }
        t1.join();
